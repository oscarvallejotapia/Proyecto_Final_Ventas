...
$mi_consulta="SELECT cliente_id, nombre, rfc
				FROM cliente
				ORDER BY nombre ASC";
...
$consulta_factura='SELECT fecha_factura, folio, cliente.cliente_id,cerrado,rfc
					FROM venta,cliente
					WHERE cliente.cliente_id=venta.cliente_id
						AND venta_id='.$mi_venta;
...
if($cerrado)
	...
	$sololectura="";
$cliente_actual=$row_fact['cliente_id'];
$rfc_actual=$row_fact['rfc'];
$fecha_actual=$row_fact['fecha_factura'];
?>

	<section id="form_box">
		<header id="header_form"> Editando una Venta <?php echo $mi_venta ?></header>
		
		<form action="venta_eliminar_editar.php?mi_id=<?php echo $mi_venta ?>" name="miformulario" id="miformulario" method="post">
			<div class="myField">
				<label for="cliente"> cliente: </label>
				<select id="cliente" name="cliente" class="largo"
													onchange="actualizaRfc();">
					<option value="-1"></option>
				<?php while($row = mysqli_fetch_assoc($result))
				{
					if($row["cliente_id"]==$cliente_actual)
						$cliente_seleccionado=" selected ";
					else
						$cliente_seleccionado="";
					echo '<option mi_rfc= "'.$row["rfc"].'" value="'.$row["cliente_id"].'"'.$cliente_seleccionado.'>';
					echo $row["nombre"];
					echo '</option>';
				} ?>
				</select>
			...
			<table id="tabla" >
				...
				<?php 
					$consulta_detalle_venta ="SELECT d.detalle_id, d.cantidad, d.precio, d.venta_id, 
					d.producto_id, p.nombre FROM detalle_venta d INNER JOIN producto p 
					ON d.producto_id = p.producto_id WHERE venta_id = " . $mi_venta;
	
					$result3 = bd_consulta($consulta_detalle_venta);
		...
		<?php 
			echo "<span id='idGeneral' hidden>" . $mi_venta . "<span>";
		?>
	</section>
